...
function scoreAcademyUnitChoice(unitType, context) {
  const { strategy, counts, currentCombatPower, knownEnemyCount, missileSiloCount, powerPlantCount, shipyardCount } = context;
  const submarineCount = counts.submarine || 0;
  const carrierCount = counts.carrier || 0;
  ...
  if (unitType === 'submarine') {
    ...
    score += assaultshipCount <= 0 ? 0.9 : 0.15;
    score += launcherCount > 0 ? 0.35 : 0;
    score += knownEnemyCount > 0 ? 0.2 : 0;
    score += powerPlantCount >= 3 ? 0.15 : -0.15;
  }
